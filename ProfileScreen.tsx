...
export function ProfileScreen() {
  ...
  // Use Firebase user data directly (name, email from Firebase Auth)
  // Use Firestore user data if available (points, stats)
  // If Firestore data not available, use defaults
  const displayName = user?.name || firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'مستخدم';
  const displayEmail = firebaseUser.email || '';
  const displayPoints = user?.points ?? 0;
  const displayReports = user?.totalReports ?? 0;
  const displayCleanings = user?.totalCleanings ?? 0;
  const displayValidations = user?.totalValidations ?? 0;

  const level = getLevelFromPoints(displayPoints);

  const menuItems = [
    { icon: <MapPin className="w-5 h-5" />, label: 'بلاغاتي', value: `${displayReports} بلاغ`, path: '/my-reports' },
    { icon: <CheckCircle className="w-5 h-5" />, label: 'عمليات التنظيف', value: `${displayCleanings} عملية`, path: '/my-cleanings' },
    { icon: <AlertTriangle className="w-5 h-5" />, label: 'عمليات التحقق', value: `${displayValidations} تحقق`, path: '/my-validations' },
  ];
      ...
      <div className="bg-gradient-to-br from-teal-600 to-teal-700 px-4 pt-8 pb-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-teal-100 text-sm">{displayEmail}</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="text-4xl font-bold text-teal-600">{displayPoints}</span>
              ...
              <div className="flex items-center gap-1">
                <div className="w-24 h-2 bg-slate-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min((displayPoints / level.maxPoints) * 100, 100)}%` }}
                  />
                </div>
              </div>
              <p className="text-xs text-slate-400 mt-1">{level.maxPoints - displayPoints} نقطة متبقية</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mt-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-xl p-4 text-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{displayReports}</p>
          ...
          <div className="bg-white rounded-xl p-4 text-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{displayCleanings}</p>
          ...
          <div className="bg-white rounded-xl p-4 text-center">
            ...
            <p className="text-2xl font-bold text-slate-900">{displayValidations}</p>
